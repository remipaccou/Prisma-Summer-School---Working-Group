...
*Same eight steps as `narrative.md`, but with the forecasting concepts and formalism made explicit
...
**Notation.**
- $y_{v,t}$ — the *observed* value of variable $v$ at time $t$ (CO₂, coal, solar, wind, nuclear, GDP).
- $\hat{y}_{j,v,t}$ — scenario $j$'s *projection* of the same thing.
- $\varepsilon_{j,v,t} = y_{v,t} - \hat{y}_{j,v,t}$ — the *forecast error*. With this sign convention,
  **positive error = the scenario projected too little** (reality came in above it).
- $F_{v,t}$ — the *predictive distribution*: the whole cloud of scenario values for $v$ at $t$.

**Four concepts the project rests on.**

1. **Conditional vs unconditional forecast.** A scenario is a *conditional* forecast,
   $\hat{y}_j = f_j(P_j)$: it says what emissions do *given* an assumed policy/technology path $P_j$
   ("if this policy, then these emissions"). An *unconditional* forecast is
   $\mathbb{E}[y_t]$ — what will actually happen, full stop. These are different objects, and you
   **cannot turn a pile of conditional "if–then" statements into an unconditional probability** just
   by counting them.
2. **Forecast origin and horizon.** Every forecast is made at an *origin* $t_0$ and looks out over a
   *horizon* $\tau = t - t_0$. Errors grow with horizon: for a random walk with an estimated trend,
   the error variance scales as $\sigma^2(\tau) \propto \tau + \tau^2/m$ (the $\tau^2$ term is the
   compounding uncertainty in the estimated trend). Short-horizon accuracy says little about the long
   horizon.
3. **Calibration vs sharpness.** A forecast is *calibrated* if the observed value falls uniformly
   across its predictive distribution — formally, the PIT $F_{v,t}(y_{v,t})$ is $\mathrm{Uniform}(0,1)$,
   i.e. reality lands near the middle of the cloud about as often as near the edges. *Sharpness* is
   how *narrow* the cloud is. The two trade off: a narrow cloud that reality keeps falling outside of
   is **overconfident**, not good.
4. **Skill vs a naive benchmark.** A forecast only earns trust if it beats a trivial rule — a random
   walk ("nothing changes") or a straight-line trend. We measure it with the
   skill ratio $= |\varepsilon_{\text{model}}| / |\varepsilon_{\text{naive}}|$: **above 1 means the
   trivial rule was closer**, i.e. the forecast added no value.
...
**Why counting does not give a probability.** Each pathway is a conditional forecast
$\hat{y}_j = f_j(P_j)$. So the 497 net-zero pathways are simply the 497 *assumed policy paths* $P_j$
that happen to end at zero — they describe the **menu the modelling teams chose to run**, not the
likelihood the real world follows. Dividing 497 by 1,564 averages over that menu as if each path were
an equally likely future. It is not.
...
## Step 2 — The hindcast: the ensemble undershoots 2025  ·  *the error series $\varepsilon$*
...
The errors $\varepsilon = y - \hat{y}$ (family-weighted ensemble mean):

| Year | $y$ (observed) | $\hat{y}$ (ensemble mean) | $\varepsilon$ |
|---|---|---|---|
...
- **2025 (+2,582, the models too low) = structural, and this one is real.** Here there is no dip to
  ...
  $\varepsilon = +2{,}582$ is the load-bearing number of the whole study — it survives every way of
  removing COVID.
...
## Step 3 — The net-zero scenarios are the most biased low  ·  *systematic bias in $\varepsilon$*

Now split the ensemble by outcome: **NZ2070** (the 497 reaching net-zero by 2070) vs **non-NZ**.
Comparing their per-scenario mean error $\mathrm{ME}_j = \frac{1}{|T|}\sum_t \varepsilon_{j,t}$
(family-weighted):
...
The net-zero pathways sit systematically *below* reality — exactly as expected, since they assumed a
faster decarbonization than has occurred.

![Net-zero scenarios are biased low, not more imprecise](nz_bias.png)

Three points keep this honest:

- **It is a bias, not imprecision.** Look at the *typical* error size (MAE): the two groups are
  close, ~6% vs ~5%. They are not far apart in *how wrong* they are; they differ in *which direction*
  they are wrong. In forecasting terms the net-zero cloud is **mis-centred, not less sharp**.
...
Skill ratio $= |\varepsilon_{\text{ensemble}}| / |\varepsilon_{\text{rule}}|$; **above 1 means the
rule won**. "% beaten" is the share of *individual* scenarios that the rule out-predicted.
...
routinely sits at the 90th or 1st percentile of $F$, then $F$ is not a probability distribution of
futures, and no $\mathbb{P}(\text{net-zero})$ can be read off it. Note that ordinary error metrics
...
error distributions overlap, it is useless for filtering. We score the separation as
$\mathrm{sep} = (\text{median}_{NZ} - \text{median}_{\text{non-NZ}}) / \mathrm{IQR}$ — how far apart
the two medians are, in units of the spread.
...
Before concluding, the boundary of what this can show. We tested skill over a 15-year horizon
($\tau = 15$); the question is about a 45-year one — and error variance grows like
$\tau + \tau^2/m$, so a 45-year claim is far less constrained than a 15-year one. Three things the
recent record structurally *cannot* settle:
...
| Random walk + error variance $\sigma^2(\tau+\tau^2/m)$ | 🟡 partial | framed in Step 8; used in the solar forecast |
